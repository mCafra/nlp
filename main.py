...
sampleInput2 = 'Donald Trump went to France to kick to the football.'
...
def speachTagging(userInput):
    a = sp(userInput)
    for word in a:
        print(f'{word.text:{12}} {word.pos_:{10}} {word.tag_:{8}} {spacy.explain(word.tag_)}')
    input()

def tagEntities(userInput):
    a = sp(userInput)
    for word in a:
        if word.ent_type_ == 'GPE' or word.ent_type_  == 'PERSON' or  word.ent_type_  == 'LOC':
            print(Fore.RED + word.text)
            Style.RESET_ALL
        else:
            Style.RESET_ALL
            print(Fore.WHITE + word.text)
    input()
...
def menu():
    ...
    print('8. Tag Named Entities - Make NLP understand famous names, countries, movies etc. so they are not picked up')
    print('9. Exit')
    print('=======================')
    a = input(": ")
    return int(a)
...
while running:
    ...
    userInput2 = formatInput(sampleInput2,0)
    menuOption = menu()
    ...
    if menuOption == 1: #Segmentation
        ...
        speachTagging(userInput)
    elif menuOption == 8:
        tagEntities(userInput2)
    elif menuOption == 9: 
        running = False
    else:
